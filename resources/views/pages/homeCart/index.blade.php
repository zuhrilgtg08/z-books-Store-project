                    ...
                    <form action="{{ route('cart.update', $item->id) }}" method="POST" id="submit_form">
                        ...
                        <td>
                            <div class="col-md-6 m-auto">
                                ...
                                <input type="number" value="{{ $item->buku->stok }}" name="quantity" min="1"
                                    max="{{ $item->buku->stok }}" class="form-control text-center qty" disabled />
                                @else
                                <input type="number" value="{{ $item->quantity }}" name="quantity" min="1"
                                    max="{{ $item->buku->stok }}" class="form-control text-center qty" />
                                @endif
                            </div>
                        </td>
                        <td>
...
<div class="row justify-content-end mt-3 my-3">
    <div class="col-md-4 text-end mt-3">
        <a href="{{ route('checkout.create') }}"
            style="{{ $result->isEmpty() ? 'cursor: not-allowed !important; pointer-events: auto;' : '' }}"
            class="btn btn-primary text-end" id="checkout">
